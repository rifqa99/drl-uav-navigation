...
class DuelingDQN(nn.Module):
    def __init__(self, state_dim=783, action_dim=5, num_beams=256, frame_stack=3):
        """
        State dim calculation: (256 beams * 3 frames) + (5 kinematics * 3 frames) = 783
        """
        ...
        self.lidar_extractor = nn.Sequential(
            nn.Conv1d(in_channels=frame_stack, out_channels=16, kernel_size=5, stride=2, padding=2),
            nn.ReLU(),
        ...
        self.shared_feature_layer = nn.Sequential(
            nn.Linear(total_dense_input, 256),
            nn.ReLU()
        )

...
class AdvancedUAVDynamicsContinuous:
    ...
    def __init__(self):
        # Kinematic state parameters (Identical to Discrete Group for scientific control)
        self.dt = 0.1
    ...
    def update_physics(self, pos, vel, theta, omega, action):
        ...
        """
        # --- NEW CONTINUOUS ACTION MAPPING ---
        # Map thrust_input [0.0, 1.0] to a real force scale up to 3.0 Newtons
        # Map torque_input [-1.0, 1.0] directly to a scalar torque value between [-0.5, 0.5]
        thrust = action[0] * 3.0
        torque = action[1] * 0.5

        # 1. Stochastic Wind Noise Addition (Identical)
